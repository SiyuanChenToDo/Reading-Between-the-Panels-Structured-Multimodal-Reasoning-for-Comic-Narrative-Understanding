"""
Train a multimodal temporal ordering model v2.
Fixes for v1 collapse:
- Shuffle panel order during training
- Add spatial embeddings + language embeddings
- Use pairwise ranking loss + ListMLE
- Deeper score head, dropout
"""
import json
import os
import numpy as np
...
class ComicOrderModel(nn.Module):
    def __init__(self, vocab_size, d_model=768, nhead=8, num_layers=3, dim_feedforward=2048, dropout=0.1):
        super().__init__()
        self.text_encoder = TextEncoder(vocab_size, output_dim=d_model)
        self.lang_embed = nn.Embedding(3, d_model)  # 0=unk, 1=zh, 2=en
        self.spatial_embed = nn.Embedding(4, d_model)  # spatial position: TL, TR, BL, BR
...
def list_mle_loss(scores, target_perm):
    loss = 0.0
    for i in range(4):
        remaining = target_perm[:, i:]
        remaining_scores = torch.gather(scores, 1, remaining)
        correct_score = remaining_scores[:, 0]
        loss += (torch.logsumexp(remaining_scores, dim=1) - correct_score).mean()
    return loss / 4.0


def pairwise_ranking_loss(scores, target_perm):
    B = scores.size(0)
    device = scores.device
    
    # rank[b, p] = position of panel p in the target order (0=first)
    ranks = torch.zeros(B, 4, dtype=torch.long, device=device)
    for b in range(B):
        for pos, panel in enumerate(target_perm[b]):
            ranks[b, panel] = pos
    
    # Build pairwise comparison mask
    # For all pairs (i, j), we want score[i] > score[j] if rank[i] < rank[j]
    # scores: [B, 4]
    # We want: score[i] - score[j] > 0 when rank[i] < rank[j]
    si = scores.unsqueeze(2)  # [B, 4, 1]
    sj = scores.unsqueeze(1)  # [B, 1, 4]
    diff = sj - si  # [B, 4, 4], diff[i,j] = score[j] - score[i]
    
    ri = ranks.unsqueeze(2)  # [B, 4, 1]
    rj = ranks.unsqueeze(1)  # [B, 1, 4]
    
    # mask[i,j] = 1 if panel i should come before panel j
    mask = (ri < rj).float()  # [B, 4, 4]
    
    # logistic loss: log(1 + exp(diff)) for pairs where i should be before j
    # diff[i,j] = score[j] - score[i], we want this to be negative
    pair_loss = mask * F.softplus(diff)
    
    pair_count = mask.sum() + 1e-8
    return pair_loss.sum() / pair_count
...
class ComicDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            ...
            panel_feats += np.random.randn(*panel_feats.shape).astype(np.float32) * 0.05
...
def main():
    ...
    output_dir = os.path.join(script_dir, "../outputs/mm-panel-order-v2")
    ...
    model = ComicOrderModel(vocab_size=len(vocab), num_layers=3, dropout=0.1).to(device)
    ...
    for epoch in range(1, 101):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch}"):
            ...
            # Unshuffle scores so scores[:, k] = score for panel k
            unshuffle_idx = torch.zeros_like(perm)
            unshuffle_idx[perm] = torch.arange(4, device=device)
            scores = scores_shuffled[:, unshuffle_idx]
            
            loss_mle = list_mle_loss(scores, label_seq)
            loss_pair = pairwise_ranking_loss(scores, label_seq)
            loss = loss_mle + loss_pair
