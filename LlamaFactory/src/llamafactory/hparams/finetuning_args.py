...
class FinetuningArguments(
    ...
    # Story-Aware Attention Arguments (Scientific Version)
    use_story_attention: bool = field(
        default=False,
        metadata={"help": "Whether to use Story-Aware Attention for comic panel ordering tasks."},
    )
    ordering_loss_weight: float = field(
        default=0.05,
        metadata={"help": "Weight for permutation ordering auxiliary loss (lightweight)."},
    )

    # Comic Understanding Trainer Arguments
    use_comic_trainer: bool = field(
        default=False,
        metadata={"help": "Enable unified Comic Understanding Trainer with multi-scale + contrastive + story modules."},
    )
    contrastive_loss_weight: float = field(
        default=0.001,
        metadata={"help": "Weight for panel-wise contrastive loss."},
    )
    story_loss_weight: float = field(
        default=0.001,
        metadata={"help": "Weight for story-aware attention auxiliary loss."},
    )
    ...
    use_contrastive: bool = field(
        default=True,
        metadata={"help": "Enable panel-wise contrastive learning module."},
    )
    use_story_module: bool = field(
        default=True,
        metadata={"help": "Enable story-aware attention module."},
    )
    aux_warmup_steps: int = field(
        default=50,
        metadata={"help": "Warmup steps for auxiliary losses (contrastive/story). Loss weight linearly increases from 0 to 1 over this many steps."},
    )
    aux_lr_ratio: float = field(
        default=0.05,
        metadata={"help": "Learning rate ratio for auxiliary modules relative to main model (e.g., 0.05 means 5% of main LR)."},
    )
    sinkhorn_iters: int = field(
        default=20,
        metadata={"help": "Number of Sinkhorn iterations for story-aware permutation learning."},
    )
    sinkhorn_temp: float = field(
        default=0.5,
        metadata={"help": "Temperature for Sinkhorn normalization in story-aware module."},
    )
    contrastive_queue_size: int = field(
        default=128,
        metadata={"help": "Queue size for contrastive learning negative samples."},
    )
